...
## Recent Changes
- **Inventory Date Tracking (Nov 9, 2025)**: Added optional date fields to inventory form for tracking acquisition and sale dates. Inventory items can now record purchaseDate (when acquired) and soldDate (when sold) through the Admin → Business → Inventory interface. These fields use HTML5 date inputs and are optional, allowing flexible tracking of inventory lifecycle dates. Backend already supported these fields; UI now exposes them for data entry.
- **Product Deletion Fix (Nov 9, 2025)**: Fixed product deletion failing due to foreign key constraints. The deleteProduct function now properly cascades deletion: removes inventory items, nullifies product references in sales, deletes channel SKUs, product SKUs, and variants before deleting the product. Successfully tested with removal of test products.
...
- **Business Management**: Complete inventory and sales tracking system with supplier pricing (historical versioning), inventory management (individual print tracking with acquisition/sale date tracking), drop-ship order fulfillment, expense tracking with SharePoint receipt uploads, sales tax tracking (taxCollected field in cents), and multi-channel sales tracking (Website, Art Shows, Amazon, Etsy). Supports both online (sale-first) and art show (inventory-first) workflows.
