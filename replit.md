...
## Recent Changes
- **Product Sizes Management UI (Nov 8, 2025)**: Created dedicated UI for managing product sizes in Business admin panel with full CRUD operations. Added new "Sizes" tab between Suppliers and Expenses. Custom size creation now auto-calculates width, height, and aspect ratio from size label (e.g., "60x45" → width:60, height:45, ratio:4:3) using GCD algorithm. System supports whole-inch dimensions (standard for photography prints). Added 60x45 size for 4:3 aspect ratio products. Note: Decimal sizes (8.5x11) would require database schema changes; current implementation rounds to nearest inch, which is industry-standard for fine art photography prints.
